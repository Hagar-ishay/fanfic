...
export async function translateChapter(chapterText: string, chapterTitle: string) {
  try {
    const response = await model.generateContentStream(`${translationBookPrompt}\n${chapterText}`);
    let chapterChunks = "";
...
export async function translateMetadata(contents: object) {
  ...
  const result = await model.generateContent(`${translationBookMetadataPrompt}\n${metadata}`);
